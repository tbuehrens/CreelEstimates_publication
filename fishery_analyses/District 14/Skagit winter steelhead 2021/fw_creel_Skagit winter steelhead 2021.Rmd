...
season_results<-data.frame(res$C_sum,res$E_sum)%>%
  ...
  mutate(`Seasonal CPUE` = `Season Total Catch`/`Season Total Effort`)%>%
  pivot_longer(names_to = "Parameter",values_to="value",cols=c(`Season Total Catch`,`Season Total Effort`,`Seasonal CPUE`))
...
total_CPUE_plt <- ggplot(season_results %>% 
  ...
  geom_vline(data = season_results %>%
             ...
             aes(xintercept = value),
             linetype = "dashed") +
  theme(legend.title = element_blank(), lege)
...
prob_management_plt<-gridExtra::grid.arrange(
