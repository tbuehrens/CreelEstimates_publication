...
summary_stats%>%
  pivot_wider(names_from = stats,values_from = value)%>%
  filter(parameter=="C_sum")%>%
  arrange(I,desc(E))

summary_stats%>%
  pivot_wider(names_from = stats,values_from = value)%>%
  filter(parameter=="E_sum")%>%
  arrange(I,desc(E))



ggplot(summary_stats,aes(y=value,x=factor(E),color=factor(I),group=interaction(parameter,stats)))+
  facet_wrap(stats~parameter,scales="free")+
  geom_point()
